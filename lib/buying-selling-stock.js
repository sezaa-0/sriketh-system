...
export function computeBssMetrics(form) {
  const buyingWeight = toNum(form.buying_weight);
  const sellingWeight = toNum(form.selling_weight);
  ...
  const totalBuyingAmount = buyingWeight * buyingPricePerKg;
  const totalSellingAmount = sellingWeight * sellingPricePerKg;
...
export function getBssCommodityLabel(row) {
  const type = String(row?.commodity_type ?? "").trim() || "—";
  const variety = getBssVariety(row);
  if (!variety || variety === "—") return type;
  return `${type} — ${variety}`;
}

export function formatBssWeightKg(value) {
  const n = toNum(value);
  return new Intl.NumberFormat("en-LK", { maximumFractionDigits: 2 }).format(n);
}

export function getBssBuyingWeight(row) {
  return toNum(row?.buying_weight ?? row?.buying_weight_kg ?? row?.total_kg);
}

export function getBssSellingWeight(row) {
  if (row?.selling_weight != null && row.selling_weight !== "") {
    return toNum(row.selling_weight);
  }
  return toNum(row?.buying_weight ?? row?.buying_weight_kg ?? row?.total_kg);
}

/** @deprecated Use getBssBuyingWeight or getBssSellingWeight */
export function getBssTotalKg(row) {
  return getBssBuyingWeight(row);
}
...
export function normalizeBssRow(row) {
  ...
  const normalized = {
    ...
    commodity_type: String(row.commodity_type ?? "Paddy").trim() || "Paddy",
    variety: getBssVariety(row),
    buying_weight: getBssBuyingWeight(row),
    selling_weight: getBssSellingWeight(row),
...
export function normalizeBssRecords(records) {
  return (records ?? [])
    .filter(Boolean)
    .map(normalizeBssRow)
    .filter(Boolean)
...
export function mergeBssRecord(records, record) {
  const normalized = normalizeBssRow(record);
  if (!normalized?.id) return normalizeBssRecords(records);

  const without = (records ?? []).filter((row) => row?.id !== normalized.id);
  return normalizeBssRecords([normalized, ...without]);
}
...
export function buildBssPayload(form, metrics, varietyValue) {
  ...
  return {
    ...
    buying_weight: metrics.buyingWeight,
    selling_weight: metrics.sellingWeight,
    buying_weight_kg: metrics.buyingWeight,
    total_kg: metrics.buyingWeight,
...
export function buildBssFormFromRecord(record) {
  ...
  return {
    ...
    new_variety_name: "",
    buying_weight: row.buying_weight ? String(row.buying_weight) : "",
    ...
    amount_paid_to_supplier: row.amount_paid_to_supplier
      ...
      : "",
    selling_weight: row.selling_weight ? String(row.selling_weight) : "",
    buyer_name: row.buyer_name || "",
...
export function sumBssActiveBuyingWeight(records) {
  return (records || [])
    .filter((row) => row?.is_active !== false)
    .reduce((sum, row) => sum + getBssBuyingWeight(row), 0);
}
